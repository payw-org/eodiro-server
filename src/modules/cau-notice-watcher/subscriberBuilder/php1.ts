...
export const php1: SubscriberBuilder = ({ name, key, url }) => {
    return {
        ...
        key,
        url,
        noticeItemSelector: 'table.tbl-list_new tr td.line_bottom[align="left"]:not(.bg_color1)',
        titleBuilder: (noticeElm) => noticeElm.querySelector('a').textContent.trim(),
        urlBuilder: (noticeElm) => url + '?p_mode=view&p_idx=' + /view\('([0-9]+)'\)/.exec(noticeElm.querySelector('a').href)[1]
  };
}
